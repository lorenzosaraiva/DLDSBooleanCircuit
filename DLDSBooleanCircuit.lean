...
/--
TODO: replace this axiom by an equivalence between the cover of a prefix
and assignments on the remaining `r - π.len` positions.
-/
axiom card_coverFinset
  {m r : Nat}
  (π : Prefix m r) :
  (CoverFinset π).card = m ^ (r - π.len)
